...
def find_max_dimensions_scapula(pos_array_hor_all_crop,pos_array_ver_all_crop,pos_array_prof_all_crop):
    """
...
def cropping_scapula(hor_crop_pos,ver_crop_pos,prof_crop_pos,dim_bb):
    """
    Effettua il cropping di tutte le labelmap della scapola nelle CT
    ...
    for i in range(1,1251):
        ...
        if len(hor_crop_pos[pos]) != 0: 
            ...
            cropped_ct = shoulder_seg_scapula_data[hor_crop_pos[pos][0]:hor_crop_pos[pos][1], ver_crop_pos[pos][0]:ver_crop_pos[pos][1], prof_crop_pos[pos][0]:prof_crop_pos[pos][1]]
            ...
            logger.info(f"Ritagliata scapola {i}")
            if len(hor_crop_pos[pos]) == 4: # Solo quelli con 2 spalle, faccio un altro cropping
                cropped_ct = shoulder_seg_scapula_data[hor_crop_pos[pos][2]:hor_crop_pos[pos][3], ver_crop_pos[pos][2]:ver_crop_pos[pos][3], prof_crop_pos[pos][2]:prof_crop_pos[pos][3]]
                ...
                cropped_ct = np.pad(cropped_ct,pad_width=((pad_rows_bef,pad_rows_aft),(pad_cols_bef,pad_cols_aft),(pad_prof_bef,pad_prof_aft)))
                all_cropped_cts.append(cropped_ct)
                logger.info(f"Ritagliata scapola {i} seconda volta")
...
if __name__ == "__main__":
    st = time.time()
    logger.remove()
    logger.add("scapula_preprocessing/cropping_scapula_res_log.log", format="{time:DD-MM HH:mm:ss} - {message}", level="INFO")
    logger.add(sys.stdout, format="{message}", level="SUCCESS")

    # Leggo gli array
    with open(f"scapula_preprocessing/arrays/pos_array_hor_all.pkl","rb") as f_hor,open(f"scapula_preprocessing/arrays/pos_array_ver_all.pkl","rb") as f_ver, open(f"scapula_preprocessing/arrays/pos_array_prof_all.pkl","rb") as f_prof:
    ...
    dim_bb = find_max_dimensions_scapula(pos_array_hor_all_crop,pos_array_ver_all_crop,pos_array_prof_all_crop)
    # Effettuo il cropping        
    cropped_cts = cropping_scapula(pos_array_hor_all_crop,pos_array_ver_all_crop,pos_array_prof_all_crop,dim_bb)
    logger.success(f"Tempo totale cropping: {(time.time()-st):.2f} sec")

    st = time.time()
    # Salvo il dataset
    with h5py.File("dataset_scapula_labelmap_res.hdf5","w") as f:
