    ...
    for i in range(1,1251): # Da cambiare
        ...
        if os.path.exists(f"processing/{i}/shoulder_seg_res.nii"): 
            ...
            if i not in excluded_cts:
                unique_labels = np.unique(shoulder_seg_data) # Prendo i valori unici per vedere se ci sono sia spalla destra 
                if (left_humerus_label in unique_labels) and (left_scapula_label in unique_labels): # Spalla sinistra 
                    ...
                    if pos_array_prof_scap.size != 0 and pos_array_prof_scap[-1]-pos_array_prof_scap[0] >= limit_slices: 
                        logger.info(f"Processo ct {i}: spalla sinistra")

                        pos_array_prof_ct.append(pos_array_prof_scap[0])
                        pos_array_prof_ct.append(pos_array_prof_scap[-1])    
                        # Per la scapola non è come nell'omero, prendo il valore più alto e più basso della scapola stessa
                        # sull'omero tagliato
                        ...
                        cur_slices = pos_array_prof_scap[-1] - pos_array_prof_scap[0]
...
if __name__=="__main__":
    st = time.time()
    logger.remove()
    logger.add("scapula_preprocessing/largest_box_scapula_res_log.log", format="{time:DD-MM HH:mm:ss} - {message}", level="INFO")
    pos_array_hor_all,pos_array_ver_all,pos_array_prof_all,max_width,max_height,max_slices = find_largest_box_scapula()
    logger.info(f"Tempo: {(time.time()-st):.2f} sec")

    with open(f"scapula_preprocessing/arrays/pos_array_hor_all.pkl","wb") as f_hor,open(f"scapula_preprocessing/arrays/pos_array_ver_all.pkl","wb") as f_ver, open(f"scapula_preprocessing/arrays/pos_array_prof_all.pkl","wb") as f_prof:
